            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              {stratData && <>
                <CC title="User Growth: Strategy Comparison" subtitle="How delaying profits accelerates acquisition">
                  <ResponsiveContainer width="100%" height={310}><ComposedChart margin={{ top: 5, right: 20, bottom: 5, left: 10 }}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" /><XAxis dataKey="month" data={stratData["Profit-First"]} tick={{ fontSize: 11, fill: "#9ca3af" }} /><YAxis tick={{ fontSize: 11, fill: "#9ca3af" }} tickFormatter={fmt} /><Tooltip formatter={(v: any) => [fmt(v), ""]} contentStyle={{ fontSize: 12, borderRadius: 8 }} /><Line data={stratData["Profit-First"]} dataKey="users_p50" stroke="#e94560" strokeWidth={2.5} dot={false} name="Profit-First" /><Line data={stratData["Growth-First"]} dataKey="users_p50" stroke="#10b981" strokeWidth={2.5} dot={false} name="Growth-First" /><Line data={stratData["Hybrid"]} dataKey="users_p50" stroke="#f59e0b" strokeWidth={2} dot={false} name="Hybrid" strokeDasharray="6 3" /><ReferenceLine x={commissionDelay} stroke="#10b981" strokeDasharray="3 3" label={{ value: "Commission starts", fontSize: 9, fill: "#10b981" }} /><Legend wrapperStyle={{ fontSize: 11 }} /></ComposedChart></ResponsiveContainer>
                </CC>
                <CC title="Cumulative P&L: The Cost of Growth" subtitle="Growth-first goes negative but builds larger revenue engine">
                  <ResponsiveContainer width="100%" height={310}><ComposedChart margin={{ top: 5, right: 20, bottom: 5, left: 10 }}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" /><XAxis dataKey="month" data={stratData["Profit-First"]} tick={{ fontSize: 11, fill: "#9ca3af" }} /><YAxis tick={{ fontSize: 11, fill: "#9ca3af" }} tickFormatter={fmtD} /><Tooltip formatter={(v: any) => [fmtD(v), ""]} contentStyle={{ fontSize: 12, borderRadius: 8 }} /><ReferenceLine y={0} stroke="#1a1a2e" strokeWidth={1} /><Line data={stratData["Profit-First"]} dataKey="cum_p50" stroke="#e94560" strokeWidth={2.5} dot={false} name="Profit-First" /><Line data={stratData["Growth-First"]} dataKey="cum_p50" stroke="#10b981" strokeWidth={2.5} dot={false} name="Growth-First" /><Line data={stratData["Hybrid"]} dataKey="cum_p50" stroke="#f59e0b" strokeWidth={2} dot={false} name="Hybrid" strokeDasharray="6 3" /><Legend wrapperStyle={{ fontSize: 11 }} /></ComposedChart></ResponsiveContainer>
                </CC>
              ...
              {fundData && (
                <CC title="Growth by Funding Level" subtitle="How raises translate to growth acceleration">
                  <ResponsiveContainer width="100%" height={310}><ComposedChart margin={{ top: 5, right: 20, bottom: 5, left: 10 }}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" /><XAxis dataKey="month" data={(fundData as any)["Bootstrapped"].data} tick={{ fontSize: 11, fill: "#9ca3af" }} /><YAxis tick={{ fontSize: 11, fill: "#9ca3af" }} tickFormatter={fmt} /><Tooltip formatter={(v: any) => [fmt(v), ""]} contentStyle={{ fontSize: 12, borderRadius: 8 }} />{Object.entries(fundData).map(([n, { data: d, color: c }]: any) => (<Line key={n} data={d} dataKey="p50" stroke={c} strokeWidth={2} dot={false} name={n} />))}<Legend wrapperStyle={{ fontSize: 11 }} /></ComposedChart></ResponsiveContainer>
                </CC>
              )}
